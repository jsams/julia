...
const auto host_cpu = [] {
    ...
    bool hasavx = test_bits(features[0], avx_mask);
    if (hasavx) {
        xcr0 = get_xcr0();
...
static TargetData<feature_sz> host_target_data(void)
{
    return TargetData<feature_sz>{host_cpu_name(), "", {host_cpu.second, 0},
        {feature_masks & ~host_cpu.second, 0}, 0};
}
...
static TargetData<feature_sz> arg_target_data(const TargetData<feature_sz> &arg, bool require_host)
{
    ...
    if (require_host) {
        for (size_t i = 0; i < feature_sz; i++) {
            res.en.features[i] &= host_cpu.second[i];
        }
    }
...
static uint32_t sysimg_init_cb(const void *id)
{
    ...
    if (cmdline.size() > 1) {
        ...
        if (targetarg.en.flags & JL_TARGET_CLONE_ALL)
    ...
    if (vreg_size != max_vector_size(target.en.features) &&
        (sysimg[best_idx].en.flags & JL_TARGET_VEC_CALL)) {
        if (vreg_size < 64) {
...
const std::pair<std::string,std::string> &jl_get_llvm_disasm_target(uint32_t llvmver)
{
    static const auto res = get_llvm_target_str(TargetData<feature_sz>{"generic", "",
            {feature_masks, 0}, {{}, 0}, 0}, llvmver);
    return res;
}
